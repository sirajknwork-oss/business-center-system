...
export async function GET(_request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const data = await db.collection('employees').aggregate([
      { $match: { id: params.id } },
      {
        $lookup: {
          from: 'companies',
    ...
    return NextResponse.json({ error: 'Unable to fetch employee' }, { status: 500 })
  }
}
...
export async function DELETE(_request: Request, { params }: { params: { id: string } }) {
  try {
    const { getDb } = await import('@/lib/mongodb')
    const db = await getDb()
    const result = await db.collection('employees').deleteOne({ id: params.id })
