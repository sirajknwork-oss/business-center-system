  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const staffUsers = await getEmployeesByRoles(["staff"]);
        setEmployees(staffUsers);
      } catch (err) {
        setError(err instanceof Error ? err.message : "Failed to load staff panel");
      } finally {
        setLoading(false);
      }
    }
    ...
    return (
      <div className="min-h-screen bg-zinc-50 flex items-center justify-center px-6 py-12">
        <div className="rounded-3xl border border-slate-200 bg-white/95 px-8 py-10 shadow-xl">
          <p className="text-base text-slate-700">Loading staff panel…</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-zinc-50 px-6 py-12">
      <div className="mx-auto max-w-4xl">
        <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm uppercase tracking-[0.2em] text-slate-500">Staff Panel</p>
            <h1 className="mt-3 text-3xl font-semibold text-slate-900">Staff accounts</h1>
            <p className="mt-2 text-sm leading-6 text-slate-600">
              View all staff accounts created through the creator portal.
            </p>
          </div>
        ...
        <div className="grid gap-4">
          {employees.length === 0 ? (
            <div className="rounded-3xl border border-slate-200 bg-white/95 p-8 text-center shadow-xl">
              <p className="text-lg font-semibold text-slate-900">No staff accounts found</p>
              <p className="mt-2 text-sm text-slate-600">Create staff accounts from the creator portal.</p>
            </div>
          ) : (
            employees.map((employee) => (
              <div key={employee.id} className="rounded-3xl border border-slate-200 bg-white/95 p-6 shadow-xl">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700 uppercase">Staff</span>
                </div>
              </div>
            ))
          )}
